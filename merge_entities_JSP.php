...
abstract class entityMatch {
    ...
    public abstract function find_matching_entities($entity_id,$entity_name,$src_entity_lat,$src_entity_long);
...
class entityMatchPotentialDuplicates extends entityMatch {
    public function find_matching_entities($entity_id,$entity_name,$src_entity_lat,$src_entity_long) {
...
class entityMatchLevenshtein extends entityMatch {
    ...
    public function find_matching_entities($src_entity_id,$src_entity_name,$src_entity_lat,$src_entity_long) {
        $rankings = array();
        ...
        foreach ($target_entities as $target_entity_id) {
            ...
            foreach($target_entity as $ent) {
              ...
              $this->target_entity_lats["$target_entity_id"] = $this->extract($ent,$this->lat_path,false);
              $this->target_entity_longs["$target_entity_id"] = $this->extract($ent,$this->long_path,false);
            }
         }
       ...
       foreach ($this->target_entity_names as $target_entity_id=>$target_entity_name) {
        ...
        $target = array("name"=>$target_entity_name,
                        "latitude"=>$this->target_entity_lats["$target_entity_id"],
                        "longitude"=>$this->target_entity_longs["$target_entity_id"]
                      );
       	if ($this->is_marked_duplicate($src_entity_id,$target_entity_id)) {
            		$lev=0;
            		$rankings['dup'][$target_entity_id]=$target;
                	continue;
            }
      ...
      if( $src_entity_lat and $src_entity_long and
          ...
          floatval($this->target_entity_longs["$target_entity_id"]) == floatval($src_entity_long)) {
			...
			if(count($rankings)==0) {
			     $rankings[$lev][$target_entity_id]=$target;
      }
			else {
...
foreach ($src_entities as $entity_id) {
	...
	foreach($entity as $ent) {
      ...
      $entity_lats[] = $entity_match->extract($ent,$entity_match->lat_path,false);
      $entity_longs[] = $entity_match->extract($ent,$entity_match->long_path,false);
   }

  foreach($entity_names as $key=>$entity_name) {
    $entity_lat = $entity_lats[$key];
    $entity_long = $entity_longs[$key];
  }
